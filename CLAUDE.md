...
- **Elementor is sanctioned on this project, and the global "no page builders" rule does not override it.** Top Tier Tutors is an existing Elementor site we have inherited, being migrated to code gradually. Building Elementor widgets here is the deliberate migration path, not a breach — don't re-raise it as one.
- **Elementor:** the widget in `includes/elementor/` is a thin adapter over `Blueworx_TopTierTutors_Marquee_Renderer`. Keep it that way — the renderer, shortcode, CSS and JS stay Elementor-free so the Playwright suite can drive them in a harness that has no Elementor. It is also what makes the slow migration possible: as sections move off Elementor, the shortcode renders the same markup with no widget involved.
